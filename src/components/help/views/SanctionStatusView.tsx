                ...
                <Grid>
                    ...
                    <Column justifyContent="between" size={ 7 } overflow="hidden">
                        { (sanctionInfo.sanctionReason === 'cfh.reason.EMPTY') 
                            ? <div className="col-12 fw-bold" style={{color: "var(--test-galaxytext)"}}>{ LocalizeText('help.sanction.current.none') }</div>
                            : <>
                                { ((sanctionInfo.probationHoursLeft > 0) || (sanctionInfo.isSanctionActive)) &&
                                    <div className="col-12 fw-bold" style={{color: "var(--test-galaxytext)"}}>{ LocalizeText('help.sanction.probation.reminder') }</div>
                                }
                                ...
                                <div className="col-12" style={{color: "var(--test-galaxytext)"}}>{ LocalizeText('generic.start.time') } { sanctionInfo.sanctionCreationTime }</div>
                                <div className="col-12" style={{color: "var(--test-galaxytext)"}}>{ LocalizeText('generic.reason') } { sanctionInfo.sanctionReason }</div>
                                <div className="col-12" style={{color: "var(--test-galaxytext)"}}>{ LocalizeText('help.sanction.probation.days.left') } { Math.trunc((sanctionInfo.probationHoursLeft / 24)) + 1 }</div>
                            </>
                        }
                        { ((sanctionInfo.hasCustomMute) && (!(sanctionInfo.isSanctionActive))) &&
                        <div className="col-12 fw-bold" style={{color: "var(--test-galaxytext)"}}>{ LocalizeText('help.sanction.custom.mute') }</div>
                        }
                        { (sanctionInfo.tradeLockExpiryTime && sanctionInfo.tradeLockExpiryTime.length > 0) &&
                            <div className="col-12 fw-bold" style={{color: "var(--test-galaxytext)"}}>{ LocalizeText('trade.locked.until') } { sanctionInfo.tradeLockExpiryTime }</div>
                        }
