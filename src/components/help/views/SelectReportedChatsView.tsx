    ...
    return (
        <>
            <Column gap={ 1 }>
                ...
                <Text style={{color: "var(--test-galaxytext)"}} variant="none">{ LocalizeText('help.emergency.chat_report.description') }</Text>
            </Column>
            <Column gap={ 1 } overflow="hidden">
                { !!!userChats.length &&
                    <Text style={{color: "var(--test-galaxytext)"}} variant="none">{ LocalizeText('help.cfh.error.no_user_data') }</Text> }
                { (userChats.length > 0) &&
