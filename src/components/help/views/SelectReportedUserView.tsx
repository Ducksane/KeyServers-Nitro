...
export const SelectReportedUserView: FC<{}> = props =>
{
    ...
    return (
        <>
            <Column gap={ 1 }>
                <Text style={{color: "var(--test-galaxytext)"}} variant="none" fontSize={ 4 }>{ LocalizeText('help.emergency.main.step.two.title') }</Text>
                { (availableUsers.length > 0) &&
                    <Text style={{color: "var(--test-galaxytext)"}} variant="none" >{ LocalizeText('report.user.pick.user') }</Text> }
            </Column>
            <Column gap={ 1 } overflow="hidden">
                { !!!availableUsers.length &&
                    <Text style={{color: "var(--test-galaxytext)"}} variant="none">{ LocalizeText('report.user.error.nolist') }</Text> }
                { (availableUsers.length > 0) &&
