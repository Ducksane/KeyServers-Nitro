    ...
    return (
        <Column gap={ 1 } alignItems="end">
            <Column className="nitro-infostand rounded" style={{backgroundColor: "var(--test-galaxy)"}}>
                <div className="nitro-infostand-title">
                    ...
                    <Flex alignItems="center" justifyContent="between" gap={ 1 }>
                            <Text variant="none" style={{color: "var(--test-galaxytext)"}} small wrap>{ furniData.name }</Text>
                ...
                <Column overflow="visible" className="container-fluid content-area" gap={ 1 }>
                    ...
                    <Column gap={ 1 }>
                        <Flex alignItems="center" className="bg-primary rounded py-1 px-2">
                            <img src="https://4.bp.blogspot.com/-SG4h-ciWPJY/WWf0spNEazI/AAAAAAAA7jA/VRKVjhzjmAId1nvNeG1hlneobpeeOjVtACKgBGAs/s1600/katalogi_ikoni_special_offers.gif"/> &nbsp;
                    ...
                    <Column gap={ 1 }>
                        ...
                        { (furniData.purchaseOfferId > 0) &&
                            <Flex alignItems="center" className="rounded py-1 px-2" style={{ backgroundColor: "rgb(209, 82, 84)" }}>
                                <img src="https://1.bp.blogspot.com/-4OnX3Id1PBA/WV-zzM-fUYI/AAAAAAAA6uo/T9IdzwyELNEKFi4FcY956S73uErSQ0nwQCKgBGAs/s1600/1904049800.png" /> &nbsp;
                                <Text variant="white" small pointer onClick={ event => processButtonAction('buy_one') }>
                    ...
                    <Column gap={ 1 }>
                        ...
                        { godMode &&
                            <>
                                { canSeeFurniId && <Flex alignItems="center" className="rounded py-1 px-2" style={{ backgroundColor: "rgb(64, 64, 64)" }}><Text small wrap variant="white">ID: { furniData.id }</Text></Flex> }
                                { (furniKeys.length > 0) &&
                                    <>
                                        <hr className="m-0"/>
                                        <Column gap={ 1 }>
            ...
            <Flex gap={ 1 } justifyContent="end">
                { canMove &&
                    <Button variant="none" className="text-white" style={{backgroundColor: "var(--test-galaxytwo)"}} onClick={ event => processButtonAction('move') }>
                        { LocalizeText('infostand.button.move') }
                    </Button> }
                { canRotate &&
                    <Button variant="none" className="text-white" style={{backgroundColor: "var(--test-galaxytwo)"}} onClick={ event => processButtonAction('rotate') }>
                        { LocalizeText('infostand.button.rotate') }
                    </Button> }
                { (pickupMode !== PICKUP_MODE_NONE) &&
                    <Button variant="none" className="text-white" style={{backgroundColor: "var(--test-galaxytwo)"}} onClick={ event => processButtonAction('pickup') }>
                        { LocalizeText((pickupMode === PICKUP_MODE_EJECT) ? 'infostand.button.eject' : 'infostand.button.pickup') }
                    </Button> }
                { canUse &&
                    <Button variant="none" className="text-white" style={{backgroundColor: "var(--test-galaxytwo)"}} onClick={ event => processButtonAction('use') }>
                        { LocalizeText('infostand.button.use') }
                    </Button> }
                { ((furniKeys.length > 0 && furniValues.length > 0) && (furniKeys.length === furniValues.length)) &&
                    <Button variant="none" className="text-white" style={{backgroundColor: "var(--test-galaxytwo)"}} onClick={ () => processButtonAction('save_branding_configuration') }>
                        { LocalizeText('save') }
                    </Button> }
                { ((customKeys.length > 0 && customValues.length > 0) && (customKeys.length === customValues.length)) &&
                    <Button variant="none" className="text-white" style={{backgroundColor: "var(--test-galaxytwo)"}} onClick={ () => processButtonAction('save_custom_variables') }>
                        { LocalizeText('save') }
                    </Button> }
            </Flex>
        </Column>
