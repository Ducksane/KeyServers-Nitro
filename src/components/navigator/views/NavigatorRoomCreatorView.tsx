        ...
        <Column overflow="hidden">
            <Grid overflow="hidden">
                <Column size={ 6 } gap={ 1 } overflow="auto">
                    <Column gap={ 1 }>
                        <Text variant="none" style={{color: "var(--test-galaxytext)"}}>{ LocalizeText('navigator.createroom.roomnameinfo') }</Text>
                    ...
                    <Column grow gap={ 1 }>
                        <Text variant="none" style={{color: "var(--test-galaxytext)"}}>{ LocalizeText('navigator.createroom.roomdescinfo') }</Text>
                    ...
                    <Column gap={ 1 }>
                        <Text variant="none" style={{color: "var(--test-galaxytext)"}}>{ LocalizeText('navigator.category') }</Text>
                    ...
                    <Column gap={ 1 }>
                        <Text variant="none" style={{color: "var(--test-galaxytext)"}}>{ LocalizeText('navigator.maxvisitors') }</Text>
                    ...
                    <Column gap={ 1 }>
                        <Text variant="none" style={{color: "var(--test-galaxytext)"}}>{ LocalizeText('navigator.tradesettings') }</Text>
                ...
                <Column size={ 6 } gap={ 1 } overflow="auto">
                    {
                        roomModels.map((model, index )=>
                        {
                            return (<LayoutGridItem fullHeight key={ model.name } onClick={ () => selectModel(model, index) } itemActive={ (selectedModelName === model.name) } overflow="unset" gap={ 0 } className="p-1" disabled={ (GetClubMemberLevel() < model.clubLevel) }>
                                ...
                                <Text variant="none" style={{color: "var(--test-galaxytext)"}} bold>{ model.tileSize } { LocalizeText('navigator.createroom.tilesize') }</Text>
