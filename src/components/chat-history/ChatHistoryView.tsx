            ...
            <CellMeasurer cache={ cache } columnIndex={ 0 } key={ props.key } parent={ props.parent } rowIndex={ props.index }>
                <Flex key={ props.key } style={ props.style } className="p-1" gap={ 1 }>
                    ...
                    { (item.type === ChatEntryType.TYPE_CHAT) &&
                        <>
                            <Text variant="none" style={{color: "--var(test-galaxytext)"}}  pointer noWrap dangerouslySetInnerHTML={ { __html: (item.name + ':') } } />
                            <Text variant="none" style={{color: "--var(test-galaxytext)"}}  textBreak wrap grow>{ item.message }</Text>
                        </> }
                    { (item.type === ChatEntryType.TYPE_ROOM_INFO) &&
                        <>
                            <i className="icon icon-small-room" />
                            <Text variant="none" style={{color: "--var(test-galaxytext)"}}  textBreak wrap grow>{ item.name }</Text>
                        </> }
                </Flex>
            </CellMeasurer>
        );
