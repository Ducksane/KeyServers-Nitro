...
export const ModToolsUserRoomVisitsView: FC<ModToolsUserRoomVisitsViewProps> = props =>
{
    ...
    const RowRenderer = (props: ListRowProps) =>
    {
        ...
        return (
            <Grid key={ props.key } fullHeight={ false } style={ props.style } gap={ 1 } alignItems="center" className="py-1 border-bottom">
                <Text style={{color: "var(--test-galaxytext)"}} variant="none" className="g-col-2">{ item.enterHour.toString().padStart(2, '0') }: { item.enterMinute.toString().padStart(2, '0') }</Text>
                <Text style={{color: "var(--test-galaxytext)"}} variant="none" className="g-col-7">{ item.roomName }</Text>
    ...
    return (
        <NitroCardView className="nitro-mod-tools-user-visits" theme="primary-slim" windowPosition={ DraggableWindowPosition.TOP_LEFT }>
            ...
            <NitroCardContentView className="" gap={ 1 }>
                <Column fullHeight gap={ 0 } overflow="hidden">
                    <Column gap={ 2 }>
                        <Grid gap={ 1 } className="text-black fw-bold border-bottom pb-1">
                            <Base style={{color: "var(--test-galaxytext)"}} className="g-col-2">Time</Base>
                            <Base style={{color: "var(--test-galaxytext)"}} className="g-col-7">Room name</Base>
                            <Base style={{color: "var(--test-galaxytext)"}} className="g-col-3">Visit</Base>
                        </Grid>
                    </Column>
                                ...
                                { ({ height, width }) => 
                                {
                                    return (
                                        <List
                                            ...
                                            style={{color: "var(--test-galaxytext)"}}
                                        />
                                    );
                                } }
                            </AutoSizer> }
