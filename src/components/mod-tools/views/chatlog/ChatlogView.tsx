        ...
        return (
            ...
            >
                <Grid key={ props.key } fullHeight={ false } style={ props.style } gap={ 1 } alignItems="center" className="log-entry py-1 border-bottom">
                    <Text style={{color: "var(--test-galaxytext)"}} variant="none" className="g-col-2">{ chatlogEntry.timestamp }</Text>
                    <Text style={{color: "var(--test-galaxytext)"}} variant="none" className="g-col-3" bold underline pointer onClick={ event => DispatchUiEvent(new ModToolsOpenUserInfoEvent(chatlogEntry.userId)) }>{ chatlogEntry.userName }</Text>
                    <Text style={{color: "var(--test-galaxytext)"}} variant="none" textBreak wrap className="g-col-7">{ chatlogEntry.message }</Text>
                </Grid>
            </CellMeasurer>
        );
    };
